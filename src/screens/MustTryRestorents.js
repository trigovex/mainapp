...
function MustTryRestorents(props) {
...
console.log("Admin = ",Data)
  ...
  const  GetData = async ()=>{
   ...
   fetch(Ip+'/GetItems?id='+Data.ShopId+"&itemname="+itemname+"&veg="+VegOrNonveg,{
   headers:new Headers({
  ...
  const GetResDetails=()=>{
    fetch(Ip+'/GetAdminForRes?id='+Data.ShopId,{
      headers:new Headers({
        Authorization:"Bearer " 
      })
      }).then(res=>res.json())
 ...
 console.log("Adim = ",AdminData)
  ...
  return (
    ...
    <Nav/>
        ...
        {AdminData?    <Card shoptype={AdminData.ShopType} charges={AdminData.Deliverycharges} time={AdminData.DeliveryTime} name={AdminData.ShopName} addr={AdminData.Address} photo={AdminData.ShopPhoto}/>
   :null}
     </div>
...
export default connect(mapStateToProps,{InAction,DeAction})(MustTryRestorents);